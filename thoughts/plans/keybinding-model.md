...
- [ ] Capture Daniel's actual macOS reflex set: walk through daily use and log
...
- [ ] Survey NixOS-side launcher candidates and record tradeoffs in the ticket
      ...
      `[tier: med]`
- [ ] Experiment: try the simplest mechanism first — native per-app keybinding
      ...
      `[tier: med]`
- [ ] Only if the native-config experiment leaves gaps a per-app table can't
      ...
      `[tier: high]`
- [ ] Install/configure the launcher candidate chosen from the survey on `gauss`
      via the flake, following existing module conventions in
      `hosts/gauss/default.nix`. Run `just check` after edits. `[tier: med]`
- [ ] Draft the first-pass equivalence map in the ticket table for all captured
      functions across all four app slots, based on what the experiments
      validated. `[tier: med]`
- [ ] Implement the drafted bindings on `gauss` using whichever mechanism(s) the
      experiments actually validated per function. Run `just check` after edits.
      `[tier: med]`
- [ ] Run the fidelity-ladder test procedure against the full first-pass map on
...
- [ ] Backport the finalized bindings to `hardy`, accounting for its missing
      ...
      rather than assuming it transfers unchanged. `[tier: med]`
