...
{ config, ... }:
{
  flake.modules.nixos.paperwm =
    ...
    let
      ...
      # Vicinae searches XDG data directories rather than PATH for script
      # commands. Keep both entry points in one system-owned package so removing
      # the package removes its shell and launcher surfaces together.
